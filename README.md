...
- **`md_extract_wikilinks(markdown)`** - Extract Obsidian/wiki-style links: `[[target]]`, `[[target|alias]]`, `[[target#heading]]`, `[[target^block]]`, and embeds `![[…]]`. Returns `LIST<STRUCT(target, alias, anchor, is_embed, line_number)>`. *Not* CommonMark/GFM — regex-based; see [Optional Add-On Extractors](#optional-add-on-extractors-extract_extensions).
...
`md_extract_wikilinks` and `md_extract_tags` cover the **Obsidian / wiki Markdown superset** that cmark-gfm cannot parse — `[[wikilinks]]`, `![[embeds]]`, `#tags`. They are regex-based (not part of CommonMark/GFM), and are exposed two ways:
