...
void MarkdownFunctions::RegisterValidationFunction(ExtensionLoader &loader) {
	...
	ScalarFunction md_valid_md(
	    "md_valid", {markdown_type}, LogicalType::BOOLEAN, [](DataChunk &args, ExpressionState &state, Vector &result) {
		    auto &input_vector = args.data[0];
		    UnaryExecutor::ExecuteWithNulls<string_t, bool>(
		        input_vector, result, args.size(),
		        [&](string_t md_str, ValidityMask &mask, idx_t idx) { return mask.RowIsValid(idx); });
	    });
