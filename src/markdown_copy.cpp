...
bool WriteMarkdownBindData::Equals(const FunctionData &other_p) const {
    ...
    return markdown_mode == other.markdown_mode &&
           null_value == other.null_value &&
           header == other.header &&
           escape_pipes == other.escape_pipes &&
           escape_newlines == other.escape_newlines &&
           frontmatter == other.frontmatter &&
           content_column == other.content_column &&
           title_column == other.title_column &&
           level_column == other.level_column &&
           content_mode == other.content_mode &&
    ...
    for (auto &option : options) {
        ...
        if (loption == "markdown_mode") {
            ...
            if (mode_str == "table") {
                ...
                throw InvalidInputException("Invalid markdown_mode: '%s'. Expected 'table', 'document', or 'blocks'", mode_str);
            }
...
unique_ptr<GlobalFunctionData> MarkdownCopyFunction::InitializeGlobal(ClientContext &context, FunctionData &bind_data,
    ...
    result->handle = fs.OpenFile(file_path, FileOpenFlags::FILE_FLAGS_WRITE | FileOpenFlags::FILE_FLAGS_FILE_CREATE_NEW);
...
unique_ptr<LocalFunctionData> MarkdownCopyFunction::InitializeLocal(ExecutionContext &context, FunctionData &bind_data) {
...
string MarkdownCopyFunction::RenderTableRow(const DataChunk &chunk, idx_t row_idx, const WriteMarkdownBindData &bind_data) {
    string result = "|";
    ...
    if (block_type == "frontmatter") {
        ...
        if (encoding == "json" && content.length() > 2 && content[0] == '[') {
            ...
            for (size_t i = 0; i < items_str.length(); i++) {
                ...
                if (escape_next) {
                    if (c == 'n') current_item += '\n';
                    else if (c == 't') current_item += '\t';
                    else if (c == 'r') current_item += '\r';
                    else current_item += c;
                    escape_next = false;
                } else if (c == '\\') {
                    escape_next = true;
